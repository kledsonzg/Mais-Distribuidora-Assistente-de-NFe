...
namespace NFeAssistant.HttpServer
{
    internal static class Server
    {
        ...
        private static bool IsFunctionalitiesEnabled()
        {
            try
            {
                string url = "https://github.com/kledsonzg/NFe-Assistant-Functionalities/raw/refs/heads/main/enabled.txt";
                HttpClient client = new();
                string result = client.GetStringAsync(url).Result;
                //Program.PrintLine("Resultado: " + result);
                return result == "true";
            }
            catch{}
            return false;
